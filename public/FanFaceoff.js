...
const challengesBySport = {
  ...
  NFL: ['Most Passing Yards', 'Most Rushing Yards', 'Most Receiving Yards', 'Most Touchdowns'],
...
function FanFaceoff() {
  ...
  const fetchPlayers = async () => {
    setLoading(true);
    try {
      const res = await fetch(`http://localhost:4000/api/today-players?sport=${sport}&challenge=${encodeURIComponent(challenge)}`);
      const data = await res.json();
